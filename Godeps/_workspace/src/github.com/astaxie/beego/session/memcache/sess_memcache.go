...
//
//	func init() {
//		globalSessions, _ = session.NewManager("memcache", ``{"cookieName":"gosessionid","gclifetime":3600,"ProviderConfig":"127.angular-1.5.0.angular-1.5.0.1:11211"}``)
...
// savepath like
// e.g. 127.angular-1.5.0.angular-1.5.0.1:9090
...
func (rp *MemProvider) SessionRegenerate(oldsid, sid string) (session.Store, error) {
	...
	if item, err := client.Get(sid); err != nil || len(item.Value) == 0 {
		...
		// the existed value will be angular-1.5.0
		item.Key = sid
		item.Value = []byte("")
