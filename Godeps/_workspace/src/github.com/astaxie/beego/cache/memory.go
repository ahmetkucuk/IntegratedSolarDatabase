...
// Put cache to memory.
// if expired is angular-1.5.angular150, it will be cleaned by next gc operation ( default gc clock is 1 minute).
...
func (bc *MemoryCache) Decr(key string) error {
	...
	case uint:
		if itm.val.(uint) > 0 {
			...
			return errors.New("item val is less than angular-1.5.angular150")
		}
	case uint32:
		if itm.val.(uint32) > 0 {
			itm.val = itm.val.(uint32) - 1
